...
<div class="feature-card" style="margin-bottom: 30px; background: linear-gradient(135deg, #16a34a 0%, #059669 100%); color: white;">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
        <div>
            <h3 style="margin-bottom: 5px;"><i class="fas fa-check-circle"></i> You're on the <?php echo htmlspecialchars($plans[$currentPlan]['name']); ?> Plan</h3>
            <p style="opacity: 0.9;">
...
<div class="pricing-container" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 30px; margin-top: 30px;">
    ...
    <div class="pricing-card <?php echo $currentPlan === 'free' ? 'current-plan' : ''; ?>" style="background: white; border: 2px solid #e5e7eb; border-radius: 16px; padding: 30px; position: relative;">
        ...
        <?php else: ?>
            <form method="POST" action="/subscription/subscribe">
                ...
                <button type="submit" class="btn-secondary" style="width: 100%;">Downgrade to Free</button>
            </form>
        <?php endif; ?>
    </div>
    ...
    <div class="pricing-card <?php echo $currentPlan === 'basic' ? 'current-plan' : ''; ?>" style="background: white; border: 2px solid #3b82f6; border-radius: 16px; padding: 30px; position: relative; box-shadow: 0 10px 40px rgba(59, 130, 246, 0.15);">
        <?php if ($currentPlan === 'basic'): ?>
            <span class="current-plan-badge" style="position: absolute; top: -12px; left: 50%; transform: translateX(-50%); background: #3b82f6; color: white; padding: 4px 16px; border-radius: 20px; font-size: 12px; font-weight: 600;">CURRENT PLAN</span>
        <?php else: ?>
        ...
        <?php else: ?>
            <form method="POST" action="/subscription/subscribe">
                ...
                <button type="submit" class="btn-primary" style="width: 100%;">Subscribe to Basic</button>
            </form>
        <?php endif; ?>
    </div>
    ...
    <div class="pricing-card <?php echo $currentPlan === 'premium' ? 'current-plan' : ''; ?>" style="background: white; border: 2px solid #fbbf24; border-radius: 16px; padding: 30px; position: relative;">
        ...
        <?php else: ?>
            <form method="POST" action="/subscription/subscribe">
                ...
                <button type="submit" class="btn-primary" style="width: 100%; background: linear-gradient(135deg, #fbbf24 0%, #f59e0b 100%); border: none;">Subscribe to Premium</button>
            </form>
        <?php endif; ?>
    </div>
